...
- Settings (`serverUrl`, `serverPassword`, `deliveryMode`, `autoSubmitTui`,
  `pageCharLimit`, `autoOpenPanel`) live in `chrome.storage.local` only —
  never `.sync`/`.session`. Password is never logged.
...
- `src/sidepanel/` — sessions dropdown, thread, streaming chat. Renders
  plain `pre-wrap` text only; **no markdown renderer** (XSS policy). SSE text
  parts are keyed `${messageID}:${partID}` in `partEls`.
- `src/popup/` — quick config (URL, password, delivery mode, auto-open panel).
- `src/options/` — advanced config (`pageCharLimit`, `autoSubmitTui`).
  Both pages write the same settings key; `storage.onChanged` refreshes SW and
  panel caches.
- `icons/` — committed PNGs generated by a throwaway PowerShell
  System.Drawing script (not checked in); regenerate from `dist`-side nothing,
  re-create the script if icons must change.
